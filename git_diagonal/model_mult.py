...
class MappingNets(nn.Module):
    def __init__(self,code_dim=512,n_mlp = 8,num_domains=10):
        super().__init__()
        layers = [PixelNorm()]
...
class StyledGenerator(nn.Module):
    def __init__(self, code_dim=512,num_domains=10, n_mlp=8):
        super().__init__()
...
class Discriminator(nn.Module):
    def __init__(self, fused=True, num_domains=10,from_rgb_activate=False):
        super().__init__()
        ...
        def make_from_rgb(out_channel):
            if from_rgb_activate:
                return nn.Sequential(EqualConv2d(2, out_channel, 1), nn.LeakyReLU(0.2))

            else:
                return EqualConv2d(2, out_channel, 1)
...
class StyleEncoder(nn.Module):
    def __init__(self, fused=True, num_domains=10,style_dim=512,from_rgb_activate=False):
        super().__init__()
