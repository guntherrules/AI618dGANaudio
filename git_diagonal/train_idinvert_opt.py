...
from dataset import MultiLabelResolutionDatasetOPT
...
def sample_data(batch_size, path,image_size=4):
    dataset = MultiLabelResolutionDatasetOPT(path,resolution=image_size)
...
def train(args,  generator,encoderS,encoderP):
    ...
    for i in pbar:
        ...
        for ii in range(100):
            ...
            fake_image_lpips = make_3ch_lpips(fake_image)
            real_image_lpips = make_3ch_lpips(real_image)
            
            lpips_loss = calc_lpips(fake_image_lpips,real_image_lpips)
        ...
        for c in range(len(sty)):
            spath  = path_sty[y_org[c]]+'/'+str(real_image.size(0)*i + c + 1).zfill(6)+'.pt'
            torch.save(sty[c],spath)
            ppath  = path_pix[y_org[c]]+'/'+str(real_image.size(0)*i + c + 1).zfill(6)+'.pt'
        ...
        np.save(
            f'D:/DiagonalGAN/sampleREC_new/{str(i + 1).zfill(6)}',
            torch.cat(images,0)              
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dir_sty',default = 'D:/DiagonalGAN/codes/new_sty/',type=str,help='directory to save style code')
    parser.add_argument('--dir_pix',default = 'D:/DiagonalGAN/codes/new_pix/',type=str,help='directory to save content code')
    ...
    parser.add_argument(
        '--ckpt', default='./checkpoint/train_mult/train_step-latest.model', type=str, help='load from previous checkpoints'
    )
    parser.add_argument(
        '--enc_ckpt', default='./checkpoint/enc/200000.model', type=str, help='load from previous checkpoints'
    )
    parser.add_argument(
    ...
    args.pixpath = []
    for dom in domains:
        args.paths +=[args.datapath + dom]
        args.stypath +=[args.dir_sty + dom]
        args.pixpath +=[args.dir_pix + dom]
    ...
    if args.ckpt is not None:
        print('find the generator_ckpt')
        ckpt = torch.load(args.ckpt,map_location=lambda storage, loc: storage)
        g_running.load_state_dict(ckpt['g_running'])

    if args.enc_ckpt is not None:
        print('find the enc_ckpt')
