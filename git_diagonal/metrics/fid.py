...
from dataset import MultiResolutionDatasetFID
try:
...
#     device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    #     loaders = [get_eval_loader(path, img_size, batch_size) for path in paths]
        ...
        for path in paths:
            ...
            dataset = MultiResolutionDatasetFID(path,resolution=img_size)
        #     print(dataset.resolution)
            loaders += DataLoader(dataset, shuffle=True, batch_size=batch_size, num_workers=0, drop_last=True)
